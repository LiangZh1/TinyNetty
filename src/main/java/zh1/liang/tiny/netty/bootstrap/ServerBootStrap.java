...
public class ServerBootStrap extends AbstractBootstrap<ServerBootStrap,Channel>{
    ...
    private ServerBootStrap(ServerBootStrap serverBootStrap){
        ...
        synchronized (serverBootStrap.childOptions) {
            childOptions.putAll(serverBootStrap.childOptions);
        }
        synchronized (serverBootStrap.childAttrs) {
            childAttrs.putAll(serverBootStrap.childAttrs);
        }
    }
    ...
    protected void init(Channel channel) throws Exception {
        ...
        //方法中，这时候，才会把用户设置的多个handler真正添加到ChannelPipeline中。到这里，
        // pipeline.addLast(handler)这一行代码的逻辑才真正完成。
        p.addLast(new ChannelInitializer<Channel>() {
            ...
            public void initChannel(final Channel ch) throws Exception {
                ...
                //执行到这里，所有的服务端channelHandler已经添加完成了,这个是为了添加客户端的,负责客户端channel的初始化等
