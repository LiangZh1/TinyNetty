...
public class HashedWheelTimer implements Timer {
    ...
    /**
     * 这个是真正被调用的构造器，threadFactory是创建线程的工厂，tickDuration是tick的时长，也就是指针多久转一格，
     ...
     */
    public HashedWheelTimer(ThreadFactory threadFactory,
                            long tickDuration, //指针的间隔
                            TimeUnit unit,
                            int ticksPerWheel, //轮的总厂
    ...
    private static final class HashedWheelBucket{
        ...
        public void expireTimeouts(long deadline){
            ...
            while (timeout != null) {
                HashedWheelTimeout nextTask = timeout.next;
                //剩余轮数等于0就说明这一轮就可以执行该定时任务了
                if(nextTask.remainingRounds <= 0){
                    //把该定时任务从双向链表中删除，该方法的返回结果是下一个节点
                    nextTask = remove(timeout);
