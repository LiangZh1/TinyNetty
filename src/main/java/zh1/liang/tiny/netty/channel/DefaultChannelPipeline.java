...
//            };
    ...
    //主要就是两步
    //1. 将handler构造成context加入链表
    //2. 根据时机 触发handler的handlerAdd回调
    @Override
    public ChannelPipeline addFirst(EventExecutorGroup group, String name, ChannelHandler handler) {
        ...
        synchronized (this){
            ...
            //该handler才可以处理数据，那怎样才嫩让hanndler的添加状更新呢？
            // 就是在channelHandler的handlerAdded方法被回调后其状态会更新为ADD_COMPLETE
            addFirst0(newCtx);
            ...
            //这里说明channel已经register到selector了
    ...
    private ChannelHandler replace(
            final AbstractChannelHandlerContext oldCtx, String newName, ChannelHandler newHandler) {
        assert oldCtx != head && oldCtx != tail;
        final AbstractChannelHandlerContext newCtx;
        synchronized (this) {
            ...
            newCtx = newContext(oldCtx.executor, newName, newHandler);
     ...
     *
     * @param context
     * @param addedTask 是否是添加节点的任务
     */
    private void callHandlerCallbackLater(AbstractChannelHandlerContext context, boolean addedTask){
        assert !registered;

        // 如果是添加节点就创建PendingHandlerAddedTask对象，删除节点就创建PendingHandlerRemovedTask节点
        PendingHandlerCallback task =
                addedTask
                        ? new PendingHandlerAddedTask(context)
                        : new PendingHandlerRemovedTask(context);
    ...
    private abstract static class PendingHandlerCallback implements Runnable{
        ...
        PendingHandlerCallback(AbstractChannelHandlerContext context){
    ...
    private final class PendingHandlerRemovedTask extends PendingHandlerCallback{
        ...
        public PendingHandlerRemovedTask(AbstractChannelHandlerContext context) {
            super(context);
        }
                    ...
                    if (log.isWarnEnabled()) {
                        log.warn(
                                "Can't invoke handlerRemoved() as the EventExecutor {} rejected it," +
                                        " removing handler {}.", executor, context.name(), e);
                    }
                    context.setRemoved();
                }
            }
